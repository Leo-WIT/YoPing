                ...
                while (!cancellationToken.IsCancellationRequested)
                {
                    ...
                    using (var client = new TcpClient(ipVersion))
                    {
                        ...
                        try
                        {
                            ...
                            var success = result.AsyncWaitHandle.WaitOne(TimeSpan.FromMilliseconds(ApplicationOptions.PingTimeout));
                        ...
                        if (Status == ProbeStatus.Down)
                        {
                            ...
                            // Update minimum RTT.
                            stopwatch.Stop();
                            Statistics.RecordSuccess(stopwatch.ElapsedMilliseconds);
