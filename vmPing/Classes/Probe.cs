...
namespace vmPing.Classes
{
    ...
    public partial class Probe : INotifyPropertyChanged
    {
        ...
        private static bool hasShownLogWriteError;
        private static string lastLogWriteError;
        ...
        public void WriteFinalStatisticsToHistory()
        {
            ...
            AddHistory(
                $"统计：发送 {Statistics.Sent}，成功 {Statistics.Received}，丢失 {Statistics.Sent - Statistics.Received} " +
                $"（{Statistics.LossRate:0.##}% 丢包）");

            if (roundTripTimes.Count > 0)
            {
                AddHistory(
                    $"延迟：最小 {roundTripTimes.Min()}{Strings.Milliseconds_Symbol}，" +
